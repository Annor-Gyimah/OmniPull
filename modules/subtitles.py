...
# ═══════════════════════════════════════════════════════════════════════════════════
#  PART A  ──  Cleaned-up get_advanced_opts()
# ═══════════════════════════════════════════════════════════════════════════════════

# def get_advanced_opts(d) -> dict:
#     """
#     Build yt-dlp options for subtitle + comment settings stored on DownloadItem d.

#     Subtitles: yt-dlp still makes its normal one attempt first (which works
#     whenever YouTube is not rate-limiting).  If that attempt fails with 429,
#     fetch_subtitle_with_retry() in brain.py acts as the fallback.
#     """
#     opts: dict = {}

#     selected_subtitle = getattr(d, "selected_subtitle", None)
#     subtitle_format   = getattr(d, "subtitle_format",   "best")
#     is_auto_sub       = getattr(d, "is_auto_sub",       False)

#     if selected_subtitle:
#         opts["writesubtitles"]    = True
#         opts["subtitleslangs"]    = [selected_subtitle]   # must be a list
#         opts["subtitlesformat"]   = subtitle_format or "best"
#         opts["writeautomaticsub"] = bool(is_auto_sub)
...
#    # 1. Force Metadata and Chapters
#     if getattr(d, "embed_metadata", False):
#         opts["addmetadata"] = True
#         # Ensure the info-json is written as yt-dlp uses it to map metadata
#         opts["writeinfojson"] = True 

#     if getattr(d, "embed_chapters", False):
#         opts["addchapters"] = True

#     # 2. Thumbnail Post-Processor
...
#     # 3. Use the 'FFmpegMetadata' post-processor for deeper embedding
...
#         opts["audioformat"] = target_ext
#         opts["audioquality"] = "5"  # Medium VBR
#         opts["keepvideo"] = bool(getattr(d, "keep_video", False))

#     # ── 2. Remux / Recode ──
#     elif conv_mode == "Remux Video":
#         opts["remuxvideo"] = target_ext
#     elif conv_mode == "Recode Video":
#         opts["recodevideo"] = target_ext

#     # ── 3. Embedding (FFmpeg Post-processors) ──
#     if "postprocessors" not in opts:
#         opts["postprocessors"] = []
