...
def get_global_sett_folder():
    """
    Returns the appropriate global settings directory based on the OS.
...
# Set the global path in config
config.global_sett_folder = get_global_sett_folder()

def locate_setting_folder():
    """Determines the active settings folder and creates it if missing."""
...
def ensure_config_files_exist():
    """Initializes required configuration and state files if they are missing."""
    required_files = ['setting_2.cfg', 'downloads.cfg', 'queues.cfg', 'log.txt']
    for file in required_files:
        path = os.path.join(config.sett_folder, file)
        if not os.path.exists(path):
            with open(path, 'w') as f:
                if file.endswith('.cfg'):
                    # Queues and Downloads are lists; Settings are dictionaries
...
def load_d_list():
    """
    Parses 'downloads.cfg' and returns a list of DownloadItem objects.
    Includes logic to sanitize task statuses based on progress and scheduling.
    """
    ctx = "LOAD-TASK"
    d_list = []
    try:
        ...
        # Convert raw dictionaries into live DownloadItem instances
        for dict_ in data:
        ...
        # Clean and validate the status of each loaded task
        for d in d_list:
            status = None
            if d.progress >= 100:
                ...
                # If in a queue, preserve active/completed or set to 'queued'
                if d.status not in (config.Status.downloading, config.Status.completed):
                    status = config.Status.queued
                else:
                    status = d.status 
            else:
                # Default fallback for un-scheduled, non-queue items
                if d.status not in (config.Status.downloading, config.Status.completed):
        ...
        log('downloads.cfg file not found', log_level=3, context=ctx)
    except Exception as e:
        log(f'Error in load_d_list: {e}', log_level=3, context=ctx)
    finally:
...
def save_d_list(d_list):
    """Serializes the current download list to 'downloads.cfg'."""
    try:
        ...
        with open(file, 'w') as f:
            try:
                ...
                log(f'Error saving downloads.cfg: {e}', log_level=3)
    except Exception as e:
        handle_exceptions(e)
     

def load_setting():
    """Loads application settings and updates the global config module."""
    settings = {}
    try:
        file = os.path.join(config.sett_folder, 'setting_2.cfg')
        with open(file, 'r') as f:
            settings = json.load(f)
    except FileNotFoundError:
        log('setting_2.cfg not found', log_level=2)
    except Exception as e:
        handle_exceptions(e)
    finally:
...
def save_setting():
    """Extracts specific keys from config and saves them to 'setting_2.cfg'."""
...
def load_queues():
    """Retrieves the list of custom download queues from disk."""
    try:
...
def save_queues(queues):
    """Saves the current queue configuration to disk."""
    try:
