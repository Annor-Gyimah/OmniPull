...
from modules.utils import size_format
...
class PopulateTableWorker(QObject):
    ...
    def run(self):
        ...
        for d in reversed(self.d_list):
            ...
            # build display-ready 'done' string (this can also keep raw numeric if UI expects it)
            ...
            row_data = {
                ...
                'status': d.status,
                'i': "✔"  if d.status == Status.completed else d.i, 
                'folder': getattr(d, 'folder', ''),
                'last_try_date': getattr(d, 'last_try_date', None),
            }
